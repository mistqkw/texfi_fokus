...
/// Разновидность обычного дрифера. Не окраска одного силуэта, а восемнадцать
/// разных существ: у каждого свой спрайт, свои пропорции и свой характер.
///
/// По три на мир, и ни одного повтора между мирами. Повтор одной и той же
/// тройки — самая заметная причина, по которой карта читается как заглушка:
/// пройдя первый мир, человек во втором встречает ровно тех же противников
/// под теми же именами, и «продвижение» перестаёт что-либо значить.
///
...
enum DrifterSpecies {
  ...
  /// Эхо — фигура и две её выцветающие копии со сдвигом. То же самое,
  /// сказанное ещё раз и ещё раз.
  echo,

  /// Список — стопка одинаковых строк, снизу нарастает новая. Вычеркнутое
  /// не кончает список, а освобождает в нём место.
  ledger,

  /// Маятник — подвес и груз. Уходит и возвращается ровно туда же.
  pendulum,

  // --- Мир 5 ---

  /// Зеркало — рама, в которой стоит смутный силуэт, повторяющий движения.
  mirror,

  /// Цвет — раскрытые лепестки вокруг тёмной сердцевины. Красивое, на что
  /// уходит время.
  bloom,

  /// Шип — мелкий и кривой, сидит в углу кадра. По отдельности пустяк,
  /// цепляет каждый раз.
  thorn,

  // --- Мир 6 ---

  /// Уголёк — ядро и зола вокруг. Не помеха снаружи, а то, что осталось от
  /// собственного внимания.
  ember,

  /// Груз — низкий тяжёлый блок, вдавленный в нижний край кадра.
  weight,

  /// Дверь — проём, за которым такая же комната.
  door;
...
abstract final class GameRules {
  ...
  /// не кончаются. Тот же день — про творчество, которое возвращается к
  /// одному и тому же; зеркальный сад — про спорт, где сравниваешь себя с
  /// собой.
  ///
  /// Последняя комната возвращается к учёбе, и это единственный повтор в
  /// списке — намеренный. Круг замыкается там же, где начался, и сама
  /// категория здесь читается иначе: в первом мире учёба — это как садятся
  /// за стол, в шестом — что от этого остаётся.
  ///
  /// «Прочее» не досталось ни одному миру, и не могло: [resonates]
  /// исключает его как значение по умолчанию, а не выбор. Мир с такой
  /// привязкой не откликался бы никогда — то есть привязки у него не было
  /// бы вовсе, только строчка в коде.
  static const List<WorldDefinition> worlds = [
    ...
    WorldDefinition(
      roster: [
        DrifterSpecies.echo,
        DrifterSpecies.ledger,
        DrifterSpecies.pendulum,
      ],
      affinity: TaskCategory.creative,
    ),
    WorldDefinition(
      roster: [
        DrifterSpecies.mirror,
        DrifterSpecies.bloom,
        DrifterSpecies.thorn,
      ],
      affinity: TaskCategory.sport,
    ),
    WorldDefinition(
      roster: [
        DrifterSpecies.ember,
        DrifterSpecies.weight,
        DrifterSpecies.door,
      ],
      affinity: TaskCategory.study,
    ),
  ...
  /// Сколько миров в одном королевстве.
  ///
  /// Два, а не три и не четыре. Королевство — это не порция контента, а
  /// пара: два разных способа проиграть одному и тому же. Тихая комната и
  /// громкое поле — два способа не начать; тот же день и длинный зал — два
  /// способа не дойти. С одним миром пары нет, с тремя она перестаёт
  /// читаться как пара.
  static const int worldsPerKingdom = 2;

  /// Сколько королевств получается из написанных миров.
  ///
  /// Считается, а не задаётся: неполное королевство — это нормально, оно
  /// просто короче на карте. Задать число отдельно значило бы завести
  /// второй источник правды про одно и то же.
  static int get kingdomCount =>
      (worldCount + worldsPerKingdom - 1) ~/ worldsPerKingdom;

  /// В каком королевстве стоит мир. 1-based, как и сам мир.
  static int kingdomOf(int world) =>
      ((world - 1) ~/ worldsPerKingdom) + 1;

  /// Миры одного королевства по порядку.
  ///
  /// Может вернуть один мир вместо двух — у последнего королевства, если
  /// миров написано нечётное число. Карта от этого не ломается: она рисует
  /// то, что есть.
  static List<int> worldsOfKingdom(int kingdom) {
    final first = (kingdom - 1) * worldsPerKingdom + 1;
    return [
      for (var world = first;
          world < first + worldsPerKingdom && world <= worldCount;
          world++)
        world,
    ];
  }

  /// Всего узлов на карте. Нужен там, где показывают продвижение целиком.
  static int get totalNodes => worldCount * nodesPerWorld;
