...
final timerControllerProvider =
  ...
  // Системный канал уведомления (см. [TimerAlarmSignal]) — единственная
  // гарантия сигнала в фоне и при выгруженном процессе, и его звук здесь не
  // дублируется.
  //
  // Но пока приложение открыто и на переднем плане, полагаться только на
  // него оказалось ошибкой: у части устройств и прошивок notification manager
  // приглушает или вовсе не проигрывает звук уведомления, пока его же
  // приложение находится сверху активной задачей, — и в этих условиях конец
  // сессии проходил вообще без звука, хотя пресет был выбран и звук в
  // настройках включён. Поэтому здесь снова живёт `AlarmSoundPlayer`, но
  // только как подстраховка переднего плана: он играет пресет сам, только
  // пока приложение действительно на экране (`resumed`), и делает это в
  // точности на тех же условиях, что определяют звук системы (см.
  // [timerAlarmSignalProvider]) — так эти два источника не расходятся между
  // собой в том, должен ли сигнал звучать вообще.
  final controller = TimerController(
    ...
    onAlarm: () {
      ...
      if (!resumed) return;
      final sound = ref.read(timerAlarmSignalProvider).sound;
      if (sound == null) return;
      ref.read(alarmSoundPlayerProvider).play(sound);
    },
  );
