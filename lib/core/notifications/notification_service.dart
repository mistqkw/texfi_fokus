...
class NotificationService {
  ...
  /// Убирает свои напоминания на время фокус-сессии.
  ///
  /// Системный «Не беспокоить» глушит чужие уведомления, но только там, где
  /// он есть и где на него выдан доступ. Свои приложение может убрать всегда
  /// и без разрешений — и должно: напоминание о привычке посреди сессии
  /// раздражает ровно так же, как чужое, а извиняет его только то, что оно
  /// наше.
  ///
  /// Возвращать напоминания отдельным вызовом не нужно: расписание
  /// пересобирается после каждой сессии в `syncNotifications`, и она же
  /// вызывается на старте приложения — то есть даже выгруженный посреди
  /// сессии процесс восстановит их сам.
  Future<void> suspendReminders() => _cancelReminders();
