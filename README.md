...
- 🔔 **A completion sound that plays on silent** — pick one of five bundled chiptune cues in Settings and preview it with a tap. It plays through the alarm channel (Android `USAGE_ALARM`, iOS `AVAudioSession.playback`), which the mute switch doesn't silence, alongside the notification and the vibration rather than instead of them
