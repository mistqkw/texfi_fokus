...
void main() {
  ...
  group('полутон', () {
    test('в сетке только заливка, полутон и пустота', () {
      ...
      // Значит опечатка не упала бы, а молча превратила бы полутон в
      // сплошной пиксель и съела деталь, ради которой он и заведён.
      for (final entry in all.entries) {
    ...
    test('у каждого существа есть внутренняя деталь, а не только силуэт', () {
      // То, ради чего полутон и появился: существо в двенадцати клетках
      // одной заливкой — это пятно. Проверяется буквально: у каждого
      // дрифера и босса есть хотя бы одна клетка второго тона.
      for (final entry in <String, List<String>>{...drifters, ...bosses}.entries) {
        expect(
          entry.value.any((row) => row.contains('o')),
          isTrue,
          reason: '${entry.key}: ни одной клетки полутона — это снова силуэт',
        );
      }
    });

    test('полутон не подменяет силуэт', () {
      // Обратная крайность: спрайт, где полутона больше, чем заливки,
      // теряет край и перестаёт читаться на карте, где он размером с
      // ноготь.
      //
      // Граница нестрогая из-за первой ступени аватара: искра — это ядро и
      // ореол вокруг, поровну того и другого, и это её замысел, а не
      // недосмотр. Всё, что дальше по списку, заливкой уверенно
      // перевешивает.
      for (final entry in all.entries) {
